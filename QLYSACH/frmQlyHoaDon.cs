    ...
    public partial class frmQlyHoaDon : Form
    {
        ...
        private void dtgvHD_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                string sql = "SELECT TENNV FROM NHANVIEN WHERE MANV=N'" + MaNV + "'";
        ...
        private void Sua()
        {
            ...
            if (KiemTraThongTin())
            {
                ...
                string sqlUpdate = "UPDATE HOADON SET MANV = N'" + cboNV.SelectedValue.ToString() +
                                   "', MAKH = N'" + txtMaKH.Text +
                                   "', NGAYLAPHD = '" + DateTime.ParseExact(txtNLapHD.Text, "dd/MM/yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd") +
                                   "', TONGTIEN = '" + txtTong.Text +
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            LopHamXuLy.Connect();
            string deleteSql = "DELETE FROM NHANVIEN WHERE MANV='" + txtMaHD.Text + "'";
            if (MessageBox.Show("BẠN CÓ CHẮC CHẮN MUỐN XOÁ HOÁ ĐƠN " + txtMaHD.Text + " KHÔNG ?", "XÁC NHẬN XOÁ", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                MessageBox.Show("ĐÃ XOÁ THÀNH CÔNG NHÂN VIÊN " + txtMaHD.Text, "XÁC NHẬN", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ShowHoaDon();
                Reset();
            }
            LopHamXuLy.Disconnect();
